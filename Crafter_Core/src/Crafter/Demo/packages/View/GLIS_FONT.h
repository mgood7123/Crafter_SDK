...
#ifndef CRAFTER_GLIS_FONT_H
#define CRAFTER_GLIS_FONT_H
...
public:
    ...
    Magnum::Shaders::FlatGL2D shader{Corrade::NoCreate};
    ...
    struct font_init {
        ...
        Containers::Array<unsigned char> font_memory;
        Containers::ArrayView<const char> * font_memory_tmp = nullptr;

        font_init();

        font_init(const char *path);

        font_init(const Containers::ArrayView<const char> & data);
     ...
     */
    struct atlas {
        Magnum::GL::Texture2D texture2D{Corrade::NoCreate};        // texture object
    ...
    ATLAS_TYPE * add_font(const char *id, const Containers::ArrayView<const char> & data);
